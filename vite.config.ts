...
export default defineConfig(() => {
  ...
  // Fonction helper pour créer un chemin avec le base path
  const withBase = (path: string) => {
    // Enlever le / initial si présent
    const cleanPath = path.startsWith('/') ? path.substring(1) : path
    // Enlever le ./ initial si présent
    const finalPath = cleanPath.startsWith('./') ? cleanPath.substring(2) : cleanPath
    // Retourner avec le base path (qui se termine déjà par /)
    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          ...
          screenshots: [
            {
              src: withBase('screenshot_mobile.png'),
              sizes: '616x1344',
              type: 'image/png',
              form_factor: 'narrow',
            ...
            {
              src: withBase('screenshot_desktop.png'),
              sizes: '2936x1510',
              type: 'image/png',
              form_factor: 'wide',
          ...
          icons: [
            {
              src: withBase('icons/icon-72.png'),
              sizes: '72x72',
              type: 'image/png',
              purpose: 'any'
            },
            {
              src: withBase('icons/icon-96.png'),
              sizes: '96x96',
              type: 'image/png',
              purpose: 'any'
            },
            {
              src: withBase('icons/icon-128.png'),
              sizes: '128x128',
              type: 'image/png',
              purpose: 'any'
            },
            {
              src: withBase('icons/icon-144.png'),
              sizes: '144x144',
              type: 'image/png',
              purpose: 'any'
            },
            {
              src: withBase('icons/icon-152.png'),
              sizes: '152x152',
              type: 'image/png',
              purpose: 'any'
            },
            ...
            {
              src: withBase('icons/icon-384.png'),
              sizes: '384x384',
              type: 'image/png',
              purpose: 'any'
            },
